...
async function main() {
  ...
  const duplicateReferencesRemoved = rawXmlVideoPaths.length - new Set(rawXmlVideoPaths).size;
  ...
  for (const row of rows) { const folder = String(row.vdjPath).split("/").slice(0, -1).join("/"); folders[folder] = (folders[folder] ?? 0) + 1; }
  const topFolders = Object.entries(folders).sort((a, b) => b[1] - a[1]).slice(0, 20);
  ...
  const finalReport = report.replace("## Major library folders / decades", `## Top contributing folders\n\n${topFolders.map(([folder, count]) => `- ${count}: ${folder}`).join("\n")}\n\n## Major library folders / decades`).replace("Identity resolution is exact-label-only", `## Prototype reconciliation\n\n${prototypeAudit.join("\n")}\n\nIdentity resolution is exact-label-only`);
  await writeFile(join(OUT_DIR, "coverage-report.md"), finalReport);
