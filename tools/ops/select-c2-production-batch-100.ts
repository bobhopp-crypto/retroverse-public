...
const batchRoot = process.env.C2_BATCH_ROOT ?? `reports/c2-production-batch-${batchSize}`;
const root = join(process.cwd(), batchRoot);
...
async function main() {
  ...
  const prior100 = JSON.parse(await readFile(join(process.cwd(), "reports/c2-production-batch-100/selection-manifest.json"), "utf8")).records ?? [];
  const priorRows = [...prior25, ...prior5, ...prior100];
  ...
  const selected: any[] = []; const seen = new Set<string>(); const names = [...buckets.keys()].sort((a, b) => (buckets.get(a)?.[0]?.playCount ?? 0) - (buckets.get(b)?.[0]?.playCount ?? 0));
  while (selected.length < batchSize) { let added = false; for (const name of names) { const r = buckets.get(name)?.shift(); if (!r) continue; const id = norm(r.physicalPath ?? r.vdjPath); if (seen.has(id)) continue; seen.add(id); selected.push(r); added = true; if (selected.length === batchSize) break; } if (!added) break; }
  if (selected.length !== batchSize) throw new Error(`Expected ${batchSize}, found ${selected.length}`);
