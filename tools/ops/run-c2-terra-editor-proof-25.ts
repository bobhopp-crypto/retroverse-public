...
async function main() {
  ...
  const chosen = [...new Map([selected.find((r: any) => r.status === "CLOUD_EDITOR_PASS"), failed[0], puff, vdj, unusual].filter(Boolean).map((r: any) => [r.vdjPath, r])).values()].slice(0, 5);
  if (chosen.length !== 5) throw new Error(`Expected five Terra checkpoint subjects, found ${chosen.length}`);
  const handoffs: any[] = [];
  for (const r of chosen) {
    ...
    handoffs.push({ subject: r.editorialSubject, angle: String(d.storySeed?.[0] ?? d.summary ?? `What makes ${r.editorialSubject} worth a closer listen.`), verifiedFacts: facts.slice(0, 12).map((x: any) => typeof x === "string" ? x : x.fact ?? x.claim ?? x.text ?? JSON.stringify(x)), chartFacts: d.charts?.summary && !/no chart/i.test(d.charts.summary) ? [d.charts.summary] : [], context: [d.videoPerformance?.summary, d.culturalContext?.summary].filter(Boolean), doNotClaim: d.missingAreas ?? [], sourceSubject: r.editorialSubject, comparison: { gpt4oMini: r.article, priorStatus: r.status } });
  }
  ...
  const passes = results.filter(r => r.status === "TERRA_FIRST_PASS"); const decision = passes.length >= 4 && results.every(r => !r.qualityIssues.includes("internal-leakage"));
