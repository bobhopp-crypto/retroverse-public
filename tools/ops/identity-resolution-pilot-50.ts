...
async function main() {
  ...
  const review = results.filter((r) => r.classification === "REVIEW_CANDIDATE" || r.classification === "AMBIGUOUS").map((r) => ({ videoPath: r.videoPath, vdj: `${r.vdjArtist} — ${r.vdjTitle}`, vdjArtist: r.vdjArtist, vdjTitle: r.vdjTitle, vdjYear: r.vdjYear, vdjAlbum: r.vdjAlbum, playCount: r.playCount, candidates: r.candidates, evidence: r.evidence, question: r.classification === "AMBIGUOUS" ? "Which canonical candidate and recording/version does this physical file represent?" : "Is this the standard canonical recording rather than the marked alternate version?" }));
