...
import {
  VNextWorkspace,
  type VNextGenerationResult,
} from "@/components/ops/content-creator/VNextWorkspace";
...
export function PassStudioWorkspace({
  ...
  const [generationHandoff, setGenerationHandoff] = useState<GenerationHandoff | null>(null);
  ...
  async function handleCreativeGeneration(generation: VNextGenerationResult) {
    setEventContext({ eventName: generation.event, venue: generation.venue, date: generation.date });
    const updated = await attachContentCreatorGenerationToDesignBuilder(generation.generationId);
    setTemplates((prev) => prev.map((template) => (template.id === updated.id ? updated : template)));
    setSelectedTemplateIds([updated.id]);
    setQuantities((prev) => ({
      ...prev,
      [updated.id]: prev[updated.id] ?? defaultQuantityFor(updated),
    }));
    setGenerationHandoff({
      passType: passTypeLabel(updated),
  ...
  return (
    <div className="ps-workspace">
      ...
      <div className={`ps-workspace__panel${step === "generate" ? "" : " ps-workspace__panel--hidden"}`}>
        <div className="ps-step ps-step--generate">
          <p className="ps-step__eyebrow">Step 1 of 5</p>
          <h2 className="ps-step__title">Generate</h2>
          <p className="ps-step__hint">Choose event details and creative direction, then generate new artwork.</p>
          <VNextWorkspace
            eras={eras}
            embedded
            initialPassFields={{
            ...
            onGenerationComplete={handleCreativeGeneration}
          />
        </div>
      </div>
      ...
      <div className={`ps-workspace__panel${step === "designs" ? "" : " ps-workspace__panel--hidden"}`}>
        {generationHandoff ? (
          <div className="ps-handoff" role="status" aria-live="polite">
            <span className="ps-handoff__check" aria-hidden="true">✓</span>
            <div>
              <strong>Artwork Generated</strong>
              <span>{generationHandoff.passType} Pass · Saved to Gallery</span>
              <span>Generation {generationHandoff.generationId}</span>
            </div>
          </div>
        ) : null}
        <DesignsStep
