/**
 * Historical pre-compression VNext artifact archetypes.
 *
 * Credentials deliberately consumes this catalog instead of the newer single
 * generic credential archetype. Keeping it isolated lets the restored engine
 * retain its original object variety without changing other BobOS generators.
 */

export const HISTORICAL_ARTIFACT_ARCHETYPE_IDS = [
  "festival-ticket",
  "backstage-pass",
  "press-credential",
  "record-label-promo",
...
export type HistoricalArtifactArchetypeId =
  (typeof HISTORICAL_ARTIFACT_ARCHETYPE_IDS)[number];

export type HistoricalArtifactArchetypeDef = {
  id: HistoricalArtifactArchetypeId;
  label: string;
  objectType: string;
...
export const HISTORICAL_ARTIFACT_ARCHETYPES: Record<
  HistoricalArtifactArchetypeId,
  HistoricalArtifactArchetypeDef
> = {
  "festival-ticket": {
    id: "festival-ticket",
...
export function historicalArtifactArchetypeById(
  id: HistoricalArtifactArchetypeId,
): HistoricalArtifactArchetypeDef {
  return HISTORICAL_ARTIFACT_ARCHETYPES[id];
}

export function historicalArtifactArchetypePromptBlock(
  archetype: HistoricalArtifactArchetypeDef,
  seed: number,
): string {
  return [
