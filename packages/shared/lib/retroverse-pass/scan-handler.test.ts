...
const credentials = ["RVSN000100", "RVSN000001", "RVSN500", "EVENT-2026-0001"];

for (const credential of credentials) {
  test(`scan preserves opaque credential ${credential}`, async () => {
    let received = "";
...
test("scan decodes URL encoding and trims only surrounding whitespace", async () => {
  let received = "";
  const response = await handlePassScan(
    new Request("https://retroverse.live/pass/%20EVENT-2026-0001%20"),
    "%20EVENT-2026-0001%20",
    async (value) => { received = value; return null; },
  );
