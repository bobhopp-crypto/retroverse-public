...
export function OpsCoverTrainWorkbench({
  ...
  const advanceBatch = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/ops/covers/train/advance-batch", { method: "POST" });
  ...
  const saveDecision = useCallback(
    async (decision: CoverTrainingDecisionValue) => {
      ...
      try {
        ...
        const res = await fetch("/api/ops/covers/train/decisions", {
          method: "POST",
  ...
  useEffect(() => {
    if (batch.length > 0 || advancing || advanceStarted.current) return;
    advanceStarted.current = true;
    void advanceBatch();
  }, [batch.length, advancing, advanceBatch]);
