...
export function ChapterFilmstrip(props: ChapterFilmstripProps) {
  const [frames, setFrames] = useState<FilmstripFrame[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const boundsKey = `${props.chapterId}:${props.startSec.toFixed(2)}:${props.endSec.toFixed(2)}`;
  ...
  useEffect(() => {
    ...
    void fetch(`/api/ops/media-lab/editorial/filmstrip?${params}`)
      .then(async (res) => {
  ...
  }, [boundsKey, props.chapterId, props.endSec, props.jobSlug, props.startSec, props.year]);
  ...
  return (
    <div className="ops-ml-filmstrip">
      <div className="ops-ml-filmstrip__label-row">
        <span className="ops-ml-filmstrip__label">Preview frames</span>
        <span className="ops-dim">every 10s · click to jump</span>
      </div>
