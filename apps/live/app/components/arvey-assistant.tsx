...
export function ArveyAssistant({ currentSong }: Props) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok || !data.answer) throw new Error(data.error || "ASK RETROVERSE! is unavailable.");
  ...
  return <>
    ...
    {open ? <div className="arvey-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <section className="arvey-panel" role="dialog" aria-modal="true" aria-labelledby="arvey-title">
        <div className="arvey-heading"><div><p className="arvey-kicker">A guide for the live page</p><h2 id="arvey-title">ASK RETROVERSE!</h2></div><button className="arvey-close" type="button" onClick={() => setOpen(false)} aria-label="Close Ask Retroverse">×</button></div>
