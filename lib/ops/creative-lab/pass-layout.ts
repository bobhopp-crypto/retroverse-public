...
/** QR reserve — AI-facing prompt (no inch/pixel labels that models paint as text). */
export function integratedQrZonePrompt(): string {
  return [
    `QR RESERVE ZONE (BACK ONLY — LAYOUT ONLY, NOT PRINTED TEXT):`,
    `Reserve a large clear white square in the lower half for programmatic QR at export.`,
    ...
    `2. Large clear white square — no fake modules, no texture, no readable labels inside`,
    `3. URL label band below the square (retroverse.live is overlaid at export)`,
    `4. Serial stamp at bottom edge`,
    ``,
    `Ornament outside the white square only. No duplicate QR placeholders.`,
    `Real scannable QR is composited at export — never generated by AI.`,
  ].join("\n");
}
